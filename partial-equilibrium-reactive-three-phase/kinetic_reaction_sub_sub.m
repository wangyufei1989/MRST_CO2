function n=kinetic_reaction_sub_sub(Pl,Pg,h,n,G_e,G_et,nu_e,Tk,MM)
...
% G_e        - kernel matrix for equilibrium species based on molar abundance
% G_et      -kernel matrix for equilibrium species based on mass
% nu_e      - stoichiometric matrix for the equilibrium reactions
% n         -mass of the species
% h         - mass of the components
% Tk       -temperature in K
% MM     - diagonal matrix of molar weight 
...
%%
% we have 7 equilibrium species and 4  equilibrium reactions, so we need
% 3 components. they are 
G_e(3,:)=[];%eliminate the mineral component;
G_et(3,:)=[];%eliminate the mineral component;
h([1 2])=[];

%% initial guess of moles
...
k(1)=[]; %eliminate the kinetic reaction of calcite
...
f=[G_et*MM*n(inde)-h;
    nu_e*loga(inde)-log(k)];
 f(isnan(f))=0;
 f(isinf(f))=0;
% G*n
 dnc=max(abs(dn./(n(inde)+1)));
